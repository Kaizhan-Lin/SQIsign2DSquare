export xDBL, xADD, xDBLADD, xDBLe, xTPLe, ladder, ladder3pt, x_add_sub,
    linear_comb_2_e, linear_comb_3_e, random_point, random_point_order_2power, random_point_order_l,
...
# return [3]P on Mont_A with a24 = (A + 2)/4.
function xTPL(P::Proj1{T}, a24::Proj1{T}) where T <: RingElem
    t0 = P.X - P.Z
    t2 = t0^2 
    t1 = P.X + P.Z 
    t3 = t1^2
...
# return [3^e]P on Mont_A with a24 = (A + 2)/4.
function xTPLe(P::Proj1{T}, a24::Proj1{T}, e::Int) where T <: RingElem
    outP = P
    for _ in 1:e
        outP = xTPL(outP, a24)
    end
...
# return x([a]P + [b]Q) from x(P), x(Q), x(Q-P), where ord(P) = ord(Q) = 2^e
function linear_comb_3_e(a::Integer, b::Integer, xP::Proj1{T}, xQ::Proj1{T}, xQmP::Proj1{T}, a24::Proj1{T}, e::Int) where T <: RingElem
    a = a % (BigInt(3)^e)
    b = b % (BigInt(3)^e)
    a < 0 && (a += BigInt(3)^e)
    b < 0 && (b += BigInt(3)^e)
    ...
    a = BigInt(a/(BigInt(3)^f))
    b = BigInt(b/(BigInt(3)^f))
    if a % 3 == 0
        c = invmod(b, BigInt(3)^e)
        a = (a * c) % (BigInt(3)^e)
        xR = ladder3pt(a, xQ, xP, xQmP, a24)
        xR = ladder(b, xR, a24)
    else
        c = invmod(a, BigInt(3)^e)
        b = (b * c) % (BigInt(3)^e)
        xR = ladder3pt(b, xP, xQ, xQmP, a24)
        xR = ladder(a, xR, a24)
    end
    return xTPLe(xR, a24, f)
end
...
function two_e_iso_balanced(a24::Proj1{T}, ker::Proj1{T}, e::Int, Qs::Vector{Proj1{T}}) where T <: RingElem
    F = parent(a24.X)
    K = ker
    imQs = Qs
    if e == 1
        Pd = K
        if Pd.X == 0
            a24, imQs = two_iso_zero(a24, imQs)
        else
            a24 = two_iso_curve(Pd)
            for i in 1:length(Qs)
                imQs[i] = two_iso_eval(Pd, Qs[i])   
            end
        end 
    # elseif e == 2
    #     Pd = K
    ...
    #         a24 = four_iso_curve_neg_one(a24)
    #     else
    #         a24, K1, K2, K3, = four_iso_curve(Pd)
    #         for i in 1:length(Qs)
    #             imQs[i] = four_iso_eval(K1, K2, K3, Qs[i])
    #         end
    #     end
        ...
        imQs = vcat(Qs, ker)
        a24, imQs = two_e_iso_balanced(a24, Pd, f, imQs)
        K = imQs[end]
        imQs = imQs[1:end-1]
        a24, imQs = two_e_iso_balanced(a24, K, e-f, imQs)
    end  
    return a24, imQs
end
...
# isogeny of degree 3^e
function odd_e_isogeny(a24::Proj1{T}, ker::Proj1{T}, d::Integer, e::Int, Qs::Vector{Proj1{T}}) where T <: RingElem
    F = parent(a24.X)
    K = ker
    if e == 1
        Pd = K
        a24, imQs = odd_isogeny(a24, Pd, d, Qs)
    else
        f = div(e, 2)
        Pd = K
        for i in 1:e-f
        ...
        imQs = vcat(Qs, ker)
        a24, imQs = odd_e_isogeny(a24, Pd, d, f, imQs)
        K = imQs[end]
        imQs = imQs[1:end-1]
        a24, imQs = odd_e_isogeny(a24, K, d, e-f, imQs)
    end  
    return a24, imQs
end
