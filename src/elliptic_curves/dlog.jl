...
function bi_dlog_challenge(A::T, xP::Proj1{T}, xP0::Proj1{T}, xQ0::Proj1{T}, xPQ0::Proj1{T}, E0::E0Data) where T <: RingElem
    P = Point(A, xP)
    P0 = Point(A, xP0)
    Q0 = Point(A, xQ0)
    ...
    f0 = Tate_pairing_3power(A, P0, Q0, ExponentCofactor)
    f1 = Tate_pairing_3power(A, P0, P, ExponentCofactor)
    f2 = Tate_pairing_3power(A, Q0, P, ExponentCofactor)
    ans0 = fq_dlog_power_of_3_opt(f0, E0.dlog_data_odd)
    ans1 = fq_dlog_power_of_3_opt(f1, E0.dlog_data_odd)
    ans2 = fq_dlog_power_of_3_opt(f2, E0.dlog_data_odd)
    inv_ans0 = invmod(ans0, Cofactor)
    b = (ans1 * inv_ans0) % Cofactor
    a = (-ans2 * inv_ans0) % Cofactor
    return a, b
end


# return n1, n2, n3, n4 such that P = [n1]P0 + [n1]Q0, Q = [n3]P0 + [n4]Q0
# on E_A[2^ExponentFull]
function ec_bi_dlog_response(A::T, xP::Proj1{T}, xQ::Proj1{T}, xPQ::Proj1{T}, 
    ...
    base = Weil_pairing_2power(A, Pb, Qb, ExponentFull)
    w1 = Weil_pairing_2power(A, P, Qb, ExponentFull)
    w2 = Weil_pairing_2power(A, Pb, P, ExponentFull)
    w3 = Weil_pairing_2power(A, Q, Qb, ExponentFull)
    w4 = Weil_pairing_2power(A, Pb, Q, ExponentFull)
    ...
    n0inv = invmod(n0, BigInt(2)^ExponentFull)
    n1 = (n1 * n0inv) % BigInt(2)^ExponentFull
    n2 = (n2 * n0inv) % BigInt(2)^ExponentFull
    n3 = (n3 * n0inv) % BigInt(2)^ExponentFull
    n4 = (n4 * n0inv) % BigInt(2)^ExponentFull

    return n1, n2, n3, n4
end

# return n s.t. P = [n]Q, where P, Q is a fixed point of order Cofactor and R is a point s.t. (P, R) is a basis of E_A[Cofactor]
function ec_dlog(A::T, xP::Proj1{T}, xQ::Proj1{T}, xR::Proj1{T}, E0::E0Data) where T <: RingElem
    P = Point(A, xP)
    Q = Point(A, xQ)
    R = Point(A, xR)
    f0 = Tate_pairing_3power(A, P, R, ExponentCofactor)
    f1 = Tate_pairing_3power(A, Q, R, ExponentCofactor)
    n1 = fq_dlog_power_of_3_opt(f0, E0.dlog_data_odd)
    n2 = fq_dlog_power_of_3_opt(f1, E0.dlog_data_odd)

    return (n1 * invmod(n2, Cofactor)) % Cofactor
end
...
# x^(3^e)
function triple_e(x::FqFieldElem, e::Int)
    y = x
    for i in 1:e
        y = y^3
    end
...
# make a precomputed table for dlog with base of order 3^e
function make_dlog_table_3(base::FqFieldElem, e::Int, window_size::Int)
    F = parent(base)
    l = 3^window_size
    f, r = divrem(e, window_size)
    T1 = [[F(1) for _ in 1:l] for _ in 1:(f+1)]
    ...
    for i in 1:f
        for j in 1:l-1
            T1[i+1][j+1] = triple_e(T1[i][j+1], window_size)
        end
    end

    for i in 1:f
        for j in 2:l
            T2[i][j] = triple_e(T1[i][j], r)
        end
    end
...
# compute x=[x0,x1,...,x{k-1}] s.t. h = (g^(3^r*l^(e-k)))^x
function fq_dlog_subtree_3(e::Int, h::FqFieldElem, window_size::Int,
    strategy::Vector{Int}, table::Vector{Vector{FqFieldElem}})
    t = length(strategy)
    l = BigInt(3^window_size)
    if t == 0
        h == 1 && return [0]
        for j in 1:l-1
            if h == table[end][j+1]
    ...
    L = strategy[2:t-n+1]
    R = strategy[t-n+2:t]
    hL = triple_e(h, n * window_size)
    xL = fq_dlog_subtree_3(e - n, hL, window_size, L, table)

    hR = h
    for i in 1:e-n
        hR *= table[end-e+i][xL[i]+1]
    end
    xR = fq_dlog_subtree_3(n, hR, window_size, R, table)
...
# return n such that h = g^n, where g is a fixed base of order 3^e
function fq_dlog_power_of_3_opt(h::FqFieldElem, dlog_data::DlogData)
    e, window_size, T1, T2, strategy = dlog_data.e, dlog_data.window_size, dlog_data.T1, dlog_data.T2, dlog_data.strategy
    l = BigInt(3^window_size)
    f, r = divrem(e, window_size)
    xw = fq_dlog_subtree_3(f, triple_e(h, r), window_size, strategy, T2)
...
# on E_A[Cofactor]
# Use pairings to optimize the implementation
# This can be further optimized
function bi_dlog_odd_prime_power(A::T, xP::Proj1{T}, xQ::Proj1{T}, xPQ::Proj1{T}, xR::Proj1{T}, xS::Proj1{T}, xRS::Proj1{T}, l::Int, e::Int, E0::E0Data) where T <: RingElem
    P = Point(A, xP)
    Q = Point(A, xQ)
    PQ = add(P, -Q, Proj1(A))
    if !(xPQ == Proj1(PQ.X, PQ.Z))
    ...
    f0 = Tate_pairing_3power(A, R, S, e)
    f1 = Tate_pairing_3power(A, R, P, e)
    f2 = Tate_pairing_3power(A, R, Q, e)
    f3 = Tate_pairing_3power(A, S, P, e)
    f4 = Tate_pairing_3power(A, S, Q, e)
    ans0 = fq_dlog_power_of_3_opt(f0, E0.dlog_data_odd)
    ans1 = fq_dlog_power_of_3_opt(f1, E0.dlog_data_odd)
    ans2 = fq_dlog_power_of_3_opt(f2, E0.dlog_data_odd)
    ans3 = fq_dlog_power_of_3_opt(f3, E0.dlog_data_odd)
    ans4 = fq_dlog_power_of_3_opt(f4, E0.dlog_data_odd)
    inv_ans0 = invmod(ans0, Cofactor)
    b = (ans1 * inv_ans0) % Cofactor
    d = (ans2 * inv_ans0) % Cofactor
    a = (-ans3 * inv_ans0) % Cofactor
    c = (-ans4 * inv_ans0) % Cofactor
    return a, b, c, d
end
